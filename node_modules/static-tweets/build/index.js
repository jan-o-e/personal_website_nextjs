...
var prism = require('@mapbox/rehype-prism');
...
var remarkToRehype__default = /*#__PURE__*/_interopDefaultLegacy(remarkToRehype);
var raw__default = /*#__PURE__*/_interopDefaultLegacy(raw);
...
async function fetchTweetWithPoll(tweetId) {
  ...
  if (!process.env.TWITTER_ACCESS_TOKEN || !twitterLabsEnabled(expansions)) return null;
...
const processor = unified__default["default"]().use(markdown__default["default"]).use(remarkToRehype__default["default"], {
  handlers,
  allowDangerousHtml: true
})
// Add custom HTML found in the tweet to the AST
.use(raw__default["default"])
// Add syntax highlighting
.use(prism__default["default"]).use(toAst);
...
function visitAnchor(node) {
  ...
  if (!isAbsoluteUrl) {
    node.properties.href = url.resolve(TWITTER_URL, href);
  }
}
function rehypeTweet(context) {
  ...
  function visitData(node) {
    ...
    if (ctx != null && ctx.data) node.data = ctx.data;
...
// Create the processor, the order of the plugins is important
const getProcessor = unified__default["default"]().use(parse__default["default"])
// Sanitize the HTML
.use(sanitize__default["default"], githubSchema).use(rehypeMinify).freeze();
...
class Context {
  constructor() {
    this.slugger = new GithubSlugger__default["default"]();
    this.map = [];
  }
  get(id) {
